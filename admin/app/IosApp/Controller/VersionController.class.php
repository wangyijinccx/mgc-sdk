...
//             ))->find();
    ...
    public function genConfig($isConfig=true) {
        ...
        $pinyin = base64_encode($initial.'/256');
        $opt = md5(md5($initial.'/256'.$initial).'resub');

        $agentgame = base64_encode($initial);
        $opt = base64_encode($opt);
        $reurl = 'http://gm.2lyx.com/ios/index.html';
        $data_string = array(
            'p'     => $pinyin,
            'a'     => $agentgame,
            'o'     => $opt
        ...
        if ($isConfig){
            $data_string['image'] = DOWNIP.'/sdkgame/logo.jpg';
