...
class CFPropertyList extends CFBinaryPropertyList implements Iterator {
     ...
     * @param integer $format he format of the property list, see {@link FORMAT_XML}, {@link FORMAT_BINARY} and {@link
     *                        FORMAT_AUTO}, defaults to {@link FORMAT_AUTO}
     *
     ...
     *
     * @param string  $file   Path of PropertyList, defaults to {@link $file}
     * @param integer $format The format of the property list, see {@link FORMAT_XML}, {@link FORMAT_BINARY} and {@link
     *                        FORMAT_AUTO}, defaults to {@link $format}
     *
     * @return void
    ...
    public function load($file = null, $format = null) {
        ...
        switch ($format) {
            ...
            case CFPropertyList::FORMAT_AUTO: // what we now do is ugly, but neccessary to recognize the file format
                if (is_resource($file)) {
                    if (($magic_number = fread($file, 8)) === false) {
            ...
            case CFPropertyList::FORMAT_XML:
                $doc = new DOMDocument();
                if (is_resource($file)) {
                    if (!$doc->loadXML(stream_get_contents($file))) {
    ...
    public function save($file = null, $format = null) {
        ...
        if (!in_array($format, array(self::FORMAT_BINARY, self::FORMAT_XML))) {
            throw new PListException(
                "format {$format} is not supported, use CFPropertyList::FORMAT_BINARY or CFPropertyList::FORMAT_XML"
            );
        }
        if (!file_exists($file)) {
     ...
     *
     * @param bool $formatted Print plist formatted (i.e. with newlines and whitespace indention) if true; defaults to
     *                        false
     *
    ...
    public function toXML($formatted = false) {
        ...
        $dtd = $domimpl->createDocumentType(
     ...
     * @param boolean $autoDictionary if true {@link CFArray}-detection is bypassed and arrays will be returned as
     *                                {@link CFDictionary}.
     *
