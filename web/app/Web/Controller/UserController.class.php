...
class UserController extends HomebaseController {
    ...
    public function registerinfo() {
        ...
        if ($action && $action == 'userinfo') {
            ...
            $usrpwdc = I('usrpwdc');
            // 判断密码不能为空
            if (empty($oldpwd) || empty($usrpwd) || emptY($usrpwdc)) {
                $this->error('请输入完整信息');
            }
            ...
            if (0 == strcmp($usrpwd, $oldpwd)) {
                echo "<script>alert('新密码与旧密码一致,修改失败');history.go(-1);</script>";
                exit();
            }
            ...
            $create_time = time();
            if (empty($wentione) || empty($wentitwo) || empty($wentithree)
                || empty($answerone)
                || empty($answertwo)
                || empty($answerthree)
            ) {
            ...
            if (!empty($mibao)) {
                ...
                $mb = addMibao(
                    $username, $wentione, $wentitwo, $wentithree, $answerone, $answertwo, $answerthree, $create_time
                );
            }
            if ($mb) {
                echo "<meta http-equiv='Content-Type' content='text/html; charset=UTF-8'/>";
