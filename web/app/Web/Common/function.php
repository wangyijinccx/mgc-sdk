...
function userinfo($username, $sex, $qq, $tel, $email, $mobile, $fax, $birthday, $address, $zipcode) {
    ...
    //$mdata['email'] = $email;
    //$mdata['mobile'] = $mobile;
    M('members')->where(array('username' => $username))->save($data);
 ...
 * @return date
 */
function findgamelistNew($where) {
    import('Vendor.Classes.ShowPage');
    $page = new \ShowPage();
    $page->PageSize = 8;
    $gameAsso = M("game_associated");

    //$map['adid']  = array('neq',0);
   // $map['iosid']  = array('neq',0);
    //$map['_logic'] = 'OR';

    $map["_string"] =' (adid != 0 OR iosid != 0) AND g.promote_switch = 2 AND g1.promote_switch = 2';

    $total = $gameAsso ->alias("ga")
                       ->field(" ga.id ,ga.name,ga.adid,ga.iosid,g.type,g.icon,g.create_time,g1.type type1,g1.icon icon1,g1.create_time create_time1 ")
                       ->join("LEFT JOIN ".C("DB_PREFIX")."game g ON g.id  = ga.iosid")
                       ->join("LEFT JOIN ".C("DB_PREFIX")."game g1 ON g1.id  = ga.adid")
                       ->where($map)
                       ->count();

    $page->Total = $total;
    $result = $gameAsso ->alias("ga")
        ->field(" ga.id ,ga.name,ga.adid,ga.iosid,g.type,g.icon,g.create_time,g1.type type1,g1.icon icon1,g1.create_time create_time1 ")
        ->join("LEFT JOIN ".C("DB_PREFIX")."game g ON g.id  = ga.iosid")
        ->join("LEFT JOIN ".C("DB_PREFIX")."game g1 ON g1.id  = ga.adid")
        ->where($map)
    ...
    foreach ($result as $key =>$val){
        $calassify = $val["iosid"];
        if($calassify == "0"){
            //ios id为0，没有这个游戏，或者已删除，已andorid为准
            $data[$key]["id"] = $val["adid"];
            $data[$key]["name"] = $val["name"];
            $data[$key]["type"] = $val["type1"];
            $data[$key]["icon"] = $val["icon1"];
            $data[$key]["create_time"] = $val["create_time1"];
            $data[$key]["iosid"] = $val["iosid"];
            $data[$key]["adid"] = $val["adid"];
        }else{
            //ios id不为0，就已ios为准
            $data[$key]["id"] = $val["iosid"];
            $data[$key]["name"] = $val["name"];
