...
abstract class Connection
{
    ...
    public function query($sql, $bind = [], $master = false, $class = false)
    {
        ...
        try {
            ...
            //throw new PDOException($e, $this->config, $this->queryStr);
            /* mysql 重连修改 guxiannong 2017-06-08 11:32 */
            if(false === stripos($e->getMessage(), 'server has gone away')) {
                throw new PDOException($e, $this->config, $this->queryStr);
            }
            $this->reconnect();
    ...
    public function execute($sql, $bind = [])
    {
        ...
        try {
            ...
            return $this->numRows;
        } catch (\PDOException $e) {/* mysql 重连修改 guxiannong 2017-06-08 11:32 */
           // throw new PDOException($e, $this->config, $this->queryStr);
			if(false === stripos($e->getMessage(), 'server has gone away')) {
				throw new PDOException($e, $this->config, $this->queryStr);
			}
    ...
    private function reconnect()
        {
            ...
            $this->linkWrite = null;
            $this->links = [];
            $_times=$this->reconnectTimes;
            $_times= $_times+1;
            $this->reconnectTimes =$_times;
            if($_times>3){
                Log::record($this->reconnectTimes.'重试过多次了', 'error');
                throw new PDOException($this->reconnectTimes, $this->config, $this->queryStr);
            }
            Log::record('第'.$this->reconnectTimes.'次重试', 'error');
        }
}
