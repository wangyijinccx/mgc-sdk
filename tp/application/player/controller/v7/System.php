...
//        $this->setStartsession();
        ...
        //获取浮点控制
        $_game = Db::table('c_game')->where('id',$_gol_data['app_id'])->find();
        $_rdata['float_is_show'] =$_game["float_is_show"];
        $_game_class_name = Db::table('c_game_class')->where('id', $_game['classify'])->find();
        if ($_game_class_name['name'] == 'ios') {
            $_rdata['float_is_show'] = 2;
        }
        // 轮询访问账号是否屏蔽的时间
        $_rdata['polling_time'] = 300;
        $_rdata['sdk_type'] = $_game['sdk_type'];
