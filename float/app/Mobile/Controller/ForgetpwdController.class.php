...
class ForgetpwdController extends HomebaseController {
    ...
    public function check() {
        ...
        if (3 == $userdata['status']) {
            $this->error("该用户账号已被冻结");
        }
        if (empty($userdata['email']) && empty($userdata['mobile'])) {
            $helpurl = U('Help/index');
            $msg = "<a href=$helpurl  class='blue'>该手机或邮箱未绑定账号,点此联系客服</a>";
            $this->error($msg);
        }
    ...
    public function uppwd() {
        ...
        $member_list = M('members')
            ->alias('m')
            ->join("LEFT JOIN ".C('DB_PREFIX')."mg_role mr ON m.id=mr.mem_id")
            ->join("LEFT JOIN ".C('DB_PREFIX')."game g ON mr.app_id=g.id")
            ->where($where)->field('m.username,mr.server,mr.role,g.name')->select();
        $member_data = [];
        $member = [];
            ...
            if (!isset($member[$v])) {
                ...
                if (!empty($v['username'])) {
                    $member_data[] = ['username' => $v['username'],
                                      'content'  => $v['username'].'('.$v['name'].'/'.$v['server'].'/'.$v['role'].')'];
                }
            }
        }
        $this->assign("uppwdtoken", $_SESSION['uppwd_token']);
        $this->assign("member_list", $member_data);
        $this->assign("title", '找回密码');
        $this->display();
    }
...
//                $userdata['id'] = $mem_id;
                ...
                $rs = M('members')->where($where)->save($userdata);
                if (false != $rs) {
